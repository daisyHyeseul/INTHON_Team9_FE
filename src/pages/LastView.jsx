        ...
        <div className='w-full h-full relative flex flex-col items-center p-4 text-white overflow-auto' style={{ backgroundColor: '#321E5B'}}>
            ...
            <div className="flex justify-between items-center w-5/6 max-w-md mb-4  rounded-full " style={{  border:'1px solid #7763a5', padding:'3px'}}>
                <button 
                    ...
                    className={`flex-1 rounded-full px-4 py-2  transition-all duration-500 ${view === 'myCont' ? `bg-white text-purple-800` : `bg-transparent text-white`}`}
                >
                    나의 조각글
                </button>
                <button 
                    onClick={() => setView('myComment')}
                    className={`flex-1 rounded-full px-4 py-2  transition-all duration-500 ${view === 'myComment' ? `bg-white text-purple-800` : `bg-transparent text-white`}`}
                >
                    내가 남긴 말
                </button>
            </div>
            ...
            <div  onClick={() => setIsModalOpen(true)} className="text-center flex flex-col justify-center items-center gap-2 py-3 w-5/6 rounded-xl bg-white/90 text-black" > 
                ...
                <p className="text-xs font-normal m-0">{data.summary}</p>
                <p className="text-xs font-semibold m-0">{data.ai} - {data.title}</p>
            </div>
            <img className='h-16 my-5' src="../../public/star.png" alt="" />
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 ">
                ...
                >
                    ...
                    <div className=''>
                        ...
                        <p className='text-xs font-normal'>{data.ai}</p>
                    </div>
                    <div className='border-solid border-2 w-1/3 border-purple-950'></div>
                    <p className='m-3 text-[10px]'>저는 두 갈래 길 앞에서 늘 고민하며, 어느 쪽이 맞는 길일지 몰라 방황했어요. 
