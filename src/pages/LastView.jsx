        ...
        <div className='w-full h-full relative flex flex-col items-center p-4 text-white overflow-auto' style={{ backgroundColor: '#321E5B' }}>
            ...
            <div className="flex justify-between items-center w-5/6 max-w-md mb-4 border-solid border-2 border-purple-200 rounded-full p-1">
                <button 
                    ...
                    className={`flex-1 rounded-full px-4 py-2 font-semibold transition-all duration-500 ${view === 'myCont' ? `bg-white text-purple-800` : `bg-transparent text-white`} pretendard`}
                >
                    나의 조각글
                </button>
                <button 
                    onClick={() => setView('myComment')}
                    className={`flex-1 rounded-full px-4 py-2 font-semibold transition-all duration-500 ${view === 'myComment' ? `bg-white text-purple-800` : `bg-transparent text-white`} pretendard`}
                >
                    내가 남긴 말
                </button>
            </div>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                ...
                >
                    ...
                    <div>
                        <p className='font-semibold pretendard'>{data.title}</p>
                        <p className='pretendard'>{data.writer}</p>
                    </div>
                    <div className='border-solid border-2 w-1/3 border-purple-950'></div>
                    <p className='m-3 pretendard'>저는 두 갈래 길 앞에서 늘 고민하며, 어느 쪽이 맞는 길일지 몰라 방황했어요. 
                        그래서 이 시를 통해 선택의 어려움과 혼란을 표현하고자 했습니다. 각자의 길은 다 다르고, 
                        내가 걷는 길도 결국 나만의 이야기를 만들어 주더군요. 남들과 다른 속도와 방향이지만, 그 길을 믿고 가세요. 
                        지금도 충분히 잘하고 계십니다.</p>
                    <p onClick={() => window.open('https://www.poetryfoundation.org/poems/44272/the-road-not-taken', '_blank')} className='w-full bg-purple-950 text-white py-3 rounded-3xl cursor-pointer pretendard   '>더 알아보기</p>
                </div>
            </div>
            )}
        </div>
