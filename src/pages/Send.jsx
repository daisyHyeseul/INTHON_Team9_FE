    ...
    if (
      ...
      try {
        const response = await fetch(
          'https://2d56-163-152-3-142.ngrok-free.app/api/v1/post',
          {
            method: 'POST',
            body: formData,
          }
        ...
        <form className="flex flex-col w-full space-y-4 gap-2">
          ...
          <input
            ...
            className={`text-white transition-all duration-100 form-input-tel px-5 py-2 rounded-3xl text-sm outline-none 
                            focus:border-solid focus:border-white focus:border-2  ${
          ...
          {/* 닉네임 필드 */}
          <label className="text-sm font-medium text-purple-100">닉네임</label>
          <input
            id="nickname"
            type="text"
            value={nickname}
            ...
            className={`text-white transition-all duration-100 form-input-tel px-5 py-2 rounded-3xl 
                                    ${
                                      errors.nickname && nickname === ''
                                        ? 'wrong'
                                        : ''
                                    } text-sm outline-none focus:border-solid focus:border-white focus:border-2`}
            style={{ backgroundColor: '#6C548C' }}
          />

          {/* 조각글 필드 */}
          <label className="text-sm font-medium text-purple-100">조각글</label>
          <div className="relative w-full">
            <textarea
              ...
              className={`w-full p-4 rounded-3xl focus:outline-none resize-none focus:border-solid focus:border-white focus:border-2
                            text-white scroll-smooth text-sm ${
